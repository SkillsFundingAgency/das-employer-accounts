...
public class CallToActionViewModel
{
    ...
    public int PendingReservationsCount => Reservations?.Count(x => x.Status == ReservationStatus.Pending && !x.IsExpired) ?? 0;
    ...
    public VacanciesViewModel VacanciesViewModel { get; set; } = new();
}